...
The watcher SHALL, per watch, connect to the IMAP server, authenticate, open the mailbox read-only, and run IDLE; on a mailbox change it SHALL POST a signed, content-free payload carrying the source and target identity plus the opaque resync `state` (no event type, uid, or content) to the configured notify URL. It SHALL re-IDLE periodically (per RFC 2177, roughly every 29 minutes) to keep the connection alive. The watch itself SHALL be the shared `carillon-core` `CarillonImapWatch` coroutine (the same one the CLI drives); the server owns only the transport, the reconnect, and the delivery around it. See [[server-core-reconcile]].
